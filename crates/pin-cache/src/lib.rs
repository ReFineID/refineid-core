...
/// Opaque keyed mark standing in for one card-rejected PIN value.
#[derive(Zeroize)]
...
impl PinSafetyCache {
    ...
    /// Whether `pin` has already been rejected by the card carrying `serial` for
    /// `slot`. The comparison runs in constant time.
    #[must_use]
    pub fn is_rejected(&self, serial: &[u8], slot: PinSlot, pin: &[u8]) -> bool {
    ...
    pub fn record_rejected(&mut self, serial: &[u8], slot: PinSlot, pin: &[u8]) {
    ...
    /// then the pin length ahead of the pin.
    fn fingerprint(&self, serial: &[u8], slot: PinSlot, pin: &[u8]) -> RejectedPinFingerprint {
        ...
        let Ok(serial_len) = u32::try_from(serial.len()) else {
            return RejectedPinFingerprint([0_u8; FINGERPRINT_LEN]);
        };
        let Ok(pin_len) = u8::try_from(pin.len()) else {
            return RejectedPinFingerprint([0_u8; FINGERPRINT_LEN]);
        };
...
mod tests {
    ...
    /// Two distinct full card serials, as ASCII byte strings.
    const CARD_A_SERIAL: &[u8] = b"CARD-A-FULL-SERIAL";
    const CARD_B_SERIAL: &[u8] = b"CARD-B-FULL-SERIAL";

    /// Two distinct PIN values, as their ASCII digit bytes.
    const PIN_VALUE: &[u8] = b"1357";
    const OTHER_PIN_VALUE: &[u8] = b"2468";
    ...
    fn recorded_value_is_rejected_for_its_serial_and_slot() {
        let mut cache = cache();
        cache.record_rejected(CARD_A_SERIAL, PinSlot::Pin1, PIN_VALUE);
        assert!(cache.is_rejected(CARD_A_SERIAL, PinSlot::Pin1, PIN_VALUE));
    }
    ...
    fn a_different_value_is_not_rejected() {
        let mut cache = cache();
        cache.record_rejected(CARD_A_SERIAL, PinSlot::Pin1, PIN_VALUE);
        assert!(!cache.is_rejected(CARD_A_SERIAL, PinSlot::Pin1, OTHER_PIN_VALUE));
    }
    ...
    fn a_different_slot_is_not_rejected() {
        let mut cache = cache();
        cache.record_rejected(CARD_A_SERIAL, PinSlot::Pin1, PIN_VALUE);
        assert!(!cache.is_rejected(CARD_A_SERIAL, PinSlot::Pin2, PIN_VALUE));
    }
    ...
    fn a_different_serial_is_not_rejected() {
        let mut cache = cache();
        cache.record_rejected(CARD_A_SERIAL, PinSlot::Pin1, PIN_VALUE);
        assert!(!cache.is_rejected(CARD_B_SERIAL, PinSlot::Pin1, PIN_VALUE));
    }
